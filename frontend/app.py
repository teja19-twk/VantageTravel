...
# =====================================================================
    ...
    with tab_ds:
        ...
        file_map = {
            "Climate Dataset": DATASETS_DIR / "Climate_Dataset_Final.csv",
            "Visitor Dataset": DATASETS_DIR / "spot_visitors.csv",
            "Budget Dataset": DATASETS_DIR / "trip_budget_prediction_dataset.csv"
        }

        target_file = file_map[ds_choice]
        if target_file.exists():
            df_preview = pd.read_csv(target_file, nrows=100)
            st.dataframe(df_preview, use_container_width=True)
            st.caption(f"Showing sample preview (100 rows) from {target_file.name}")
        else:
            st.info(f"Dataset file {target_file.name} is available in project root.")
    ...
    with tab_viz:
        ...
        st.markdown('<div class="step-header">📌 Step 1: Visitor Distribution by Season</div>', unsafe_allow_html=True)
        ...
        if PLOTLY_AVAILABLE and px is not None:
            ...
            df_season = pd.DataFrame({"Season": seasons, "Avg Monthly Visitors": visitors_avg}).set_index("Season")
            st.bar_chart(df_season)

        st.markdown('<div class="step-header">📌 Step 2: Trip Expenditure Breakdown (%)</div>', unsafe_allow_html=True)
        ...
        if PLOTLY_AVAILABLE and px is not None:
            fig_b = px.pie(
                ...
                color_discrete_sequence=['#0284C7', '#0D9488', '#D97706', '#059669']
            )
            st.plotly_chart(fig_b, use_container_width=True)
        else:
            df_pie = pd.DataFrame({"Category": categories, "Breakdown (%)": costs}).set_index("Category")
            st.bar_chart(df_pie)
