...
# =====================================================================
    ...
    with tab_ds:
        ...
        st.markdown("---")
        st.subheader("📋 Dataset Interactive Preview Table (Supabase Cloud)")

        ds_choice = st.selectbox("Select Dataset to Preview", ["Visitor Dataset", "Climate Dataset", "Budget Dataset"])

        supabase_table_map = {
            "Visitor Dataset": ["spot_visitors", "spot_visitors_dataset"],
            "Climate Dataset": ["climate_dataset", "climate_dataset_final", "climate_dataset_final_csv"],
            "Budget Dataset": ["trip_budget_prediction", "trip_budget_prediction_dataset", "trip_budget_log"]
        }

        def fetch_supabase_preview(choice: str):
            s_url = None
            s_key = None

            # 1. Read securely from Streamlit Cloud Secrets (st.secrets)
            try:
                if hasattr(st, "secrets") and st.secrets is not None:
                    s_url = st.secrets.get("SUPABASE_URL") or (st.secrets["SUPABASE_URL"] if "SUPABASE_URL" in st.secrets else None)
                    s_key = st.secrets.get("SUPABASE_KEY") or (st.secrets["SUPABASE_KEY"] if "SUPABASE_KEY" in st.secrets else None)
            except Exception:
                pass

            # 2. Fallback to os.getenv / environment variables
            if not s_url:
            ...
            if not s_url or not s_key:
                st.error("⚠️ **Supabase credentials missing!** Please configure `SUPABASE_URL` and `SUPABASE_KEY` in **Streamlit Cloud Secrets** (App Settings → Secrets).")
                return None, None

            # 3. Create Supabase client connection
            try:
                from supabase import create_client
                client = create_client(s_url, s_key)
            except Exception as conn_err:
                st.error(f"⚠️ **Supabase Connection Failed**: {conn_err}")
                return None, None

            candidate_tables = supabase_table_map.get(choice, [])
            last_err = None

            # 4. Query target Supabase table using .select("*").limit(100)
            for t_name in candidate_tables:
                try:
                    response = client.table(t_name).select("*").limit(100).execute()
                    if response and hasattr(response, "data") and response.data is not None:
                        return response.data, t_name
                except Exception as err:
                    last_err = err
                    continue

            fallback_t_name = choice.lower().replace(" ", "_")
            try:
                response = client.table(fallback_t_name).select("*").limit(100).execute()
                if response and hasattr(response, "data") and response.data is not None:
                    return response.data, fallback_t_name
            except Exception as err:
                last_err = err

            st.error(f"⚠️ **Supabase Table Query Error**: Failed to fetch records for '{choice}'. Details: {last_err}")
            return None, None

        with st.spinner(f"Fetching {ds_choice} records from Supabase Cloud..."):
            records, fetched_table = fetch_supabase_preview(ds_choice)

        if records is not None:
            if len(records) > 0:
                df_preview = pd.DataFrame(records)
                st.dataframe(df_preview, use_container_width=True)
                st.caption(f"Showing interactive preview ({len(records)} records, limit 100) fetched directly from Supabase table `{fetched_table}`")
            else:
                st.warning(f"⚠️ **No data available**: Supabase table `{fetched_table}` contains no records.")
