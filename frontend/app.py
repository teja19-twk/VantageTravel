    ...
    with tab_ds:
        ...
        def fetch_supabase_preview(choice: str):
            ...
            if not s_url or not s_key:
                st.error("⚠️ Supabase credentials missing! Please configure `SUPABASE_URL` and `SUPABASE_KEY` in Streamlit secrets or environment variables.")
                return None, None
            ...
            try:
                ...
                st.error(f"⚠️ Failed to connect to Supabase database: {conn_err}")
                return None, None
            ...
            st.error(f"⚠️ Failed to query Supabase table for '{choice}': {last_err}")
            return None, None

        with st.spinner(f"Fetching {ds_choice} from Supabase Cloud..."):
        ...
        if records is not None:
            if len(records) > 0:
                ...
                st.caption(f"Showing sample preview ({len(records)} rows, limit 100) fetched from Supabase table `{fetched_table}`")
            else:
                st.warning(f"No records found in Supabase table `{fetched_table}`.")
