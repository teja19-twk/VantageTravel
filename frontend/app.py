...
try:
    ...
    if s_url and s_key:
        ...
        SUPABASE_READY = False
        SUPABASE_ERROR = "SUPABASE_URL and SUPABASE_KEY must be configured in Streamlit Cloud Secrets (Manage app -> Settings -> Secrets)."
...
# =====================================================================
    ...
    with tab_ds:
        ...
        # Temporary Debug Line for Secrets verification
        try:
            sec_keys = list(st.secrets.keys()) if hasattr(st, "secrets") and st.secrets is not None else []
            st.caption(f"🔍 **Secrets available**: `{sec_keys}`")
        except Exception:
            pass
        ...
        if not SUPABASE_READY or supabase is None:
            st.error(f"⚠️ **Supabase connection failed**: {SUPABASE_ERROR}")
        else:
            with st.spinner(f"Loading {ds_choice} from Supabase..."):
                candidate_tables = supabase_table_map.get(ds_choice, [])
                records = None
                used_table = None
                last_err = None
                ...
                for t_name in candidate_tables:
                    try:
                        response = supabase.table(t_name).select("*").limit(100).execute()
                ...
                if records is None:
                    fallback_t_name = ds_choice.lower().replace(" ", "_")
                    try:
                        response = supabase.table(fallback_t_name).select("*").limit(100).execute()
                        if response and hasattr(response, "data") and response.data is not None:
                            records = response.data
                            used_table = fallback_name
                    except Exception as err:
                        last_err = err
                ...
                if records is not None:
                    if len(records) > 0:
                        ...
                        st.caption(f"Showing interactive preview ({len(records)} records, limit 100) fetched from Supabase table `{used_table}`")
                    else:
                        st.warning(f"⚠️ **No data available**: Supabase table `{used_table}` contains no records.")
                else:
                    st.error(f"⚠️ **Supabase Table Query Error**: Failed to fetch records for '{ds_choice}'. Details: {last_err}")
