...
try:
    ...
    def get_spots(district=None, category=None):
        ...
        if district and str(district).lower() != "all":
            filtered = [s for s in all_default_spots if s["district"].lower() == str(district).lower()]
...
# =====================================================================
    ...
    if "prev_dist_sel" not in st.session_state:
        st.session_state["prev_dist_sel"] = "All"
    ...
    # Reset selected spot if district changed and old spot is not in new list
    if sel_dist_global != st.session_state["prev_dist_sel"]:
        st.session_state["prev_dist_sel"] = sel_dist_global
        if "global_spot_sel" in st.session_state and st.session_state["global_spot_sel"] not in filtered_spot_names:
            st.session_state["global_spot_sel"] = filtered_spot_names[0]
