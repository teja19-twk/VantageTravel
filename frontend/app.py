    ...
    def get_spots(district=None, category=None):
        try:
            ...
            if r.status_code == 200 and "spots" in r.json():
                ...
                print(f"[Frontend Log] Spot API Status: {r.status_code}, Body: {r.text[:200]}")
        except Exception as e:
            print(f"[Frontend Log] Spot API Exception: {str(e)}")
...
# =====================================================================
    ...
    if st.button("✨ Generate Complete AI Spot Report (Climate + Crowd + Budget + Map)", type="primary", use_container_width=True):
        ...
        if spot_obj_global:
            st.write(f"**Spot Coordinates**: Latitude `{spot_lat_global}`, Longitude `{spot_lon_global}`")
            amenities_g = get_nearby_amenities(spot_name=sel_spot_global, district=spot_dist_global, lat=spot_lat_global, lon=spot_lon_global)

            # Build map markers dataframe with category symbols safely
            map_markers = [{"lat": spot_lat_global, "lon": spot_lon_global, "name": f"🎯 {sel_spot_global} (Destination)"}]
            symbol_map = {
                "hotels": "🏨",
                "restaurants": "🍽️",
                "atms": "🏧",
            ...
            for cat_key, symbol in symbol_map.items():
                for item in amenities_g.get(cat_key, []):
                    if isinstance(item, dict):
                        i_lat = item.get("lat") or item.get("latitude")
                        i_lon = item.get("lon") or item.get("longitude")
                        if i_lat is not None and i_lon is not None:
                            try:
                                i_lat = float(i_lat)
                                i_lon = float(i_lon)
                                if not (math.isnan(i_lat) or math.isnan(i_lon)):
                                    map_markers.append({
                                        "lat": i_lat,
                                        "lon": i_lon,
                                        "name": f"{symbol} {item.get('name', 'Point')} ({item.get('distance_km', 0.0)} km)"
                                    })
                            except Exception:
                                pass

            st.map(pd.DataFrame(map_markers), zoom=13)

            g_tabs = st.tabs(["🏨 Hotels", "🍽️ Restaurants", "🏧 ATMs", "🏥 Hospitals", "⛽ Fuel Stations"])
            with g_tabs[0]:
                st.dataframe(pd.DataFrame(amenities_g.get("hotels", [])), use_container_width=True)
            with g_tabs[1]:
                st.dataframe(pd.DataFrame(amenities_g.get("restaurants", [])), use_container_width=True)
            with g_tabs[2]:
                st.dataframe(pd.DataFrame(amenities_g.get("atms", [])), use_container_width=True)
            with g_tabs[3]:
                st.dataframe(pd.DataFrame(amenities_g.get("hospitals", [])), use_container_width=True)
            with g_tabs[4]:
                st.dataframe(pd.DataFrame(amenities_g.get("petrol_pumps", [])), use_container_width=True)
