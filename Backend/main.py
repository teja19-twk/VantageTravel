...
@app.get("/health")
def health_check():
    """Explicit health check endpoint for deployment monitoring."""
    db_status = check_database_connection()
    districts = get_all_districts()
    hyd_spots = get_spots(district="Hyderabad")
    from recommendation import spots_df, SPOTS_CSV
    return {
        "status": "healthy",
        "service": "VantageTravel FastAPI Backend",
        "database_connected": db_status,
        "dataset_path": str(SPOTS_CSV),
        "dataset_loaded": not spots_df.empty,
        "dataset_rows": len(spots_df),
        "district_count": len(districts),
        "hyderabad_spot_count": len(hyd_spots)
    }


@app.get("/api/debug/spots")
def api_debug_spots():
    """Diagnostic endpoint to inspect loaded dataset spots on Render."""
    districts = get_all_districts()
    hyd_spots = get_spots(district="Hyderabad")
    from recommendation import spots_df, SPOTS_CSV
    return {
        ...
        "dataset_path": str(SPOTS_CSV),
        "dataset_loaded": not spots_df.empty,
        "total_dataset_rows": len(spots_df),
        "district_count": len(districts),
        "districts": districts,
        "hyderabad_spot_count": len(hyd_spots),
        "hyderabad_spots": hyd_spots
    }
