...
def get_all_districts() -> List[str]:
    """Returns a sorted list of unique districts from datasets."""
    if not spots_df.empty and "district" in spots_df.columns:
        districts = spots_df["district"].dropna().unique().tolist()
        return sorted(list(set(str(d).strip() for d in districts if d and str(d).strip())))
