...
try:
    from config import DATA_DIR, BASE_DIR, PROJECT_ROOT, BACKEND_DIR
except ImportError:
    from database.config import DATA_DIR, BASE_DIR, PROJECT_ROOT, BACKEND_DIR

# Load datasets safely with candidate resolution across Linux/Render working directories
spots_csv_candidates = [
    DATA_DIR / "other spots.csv",
    DATA_DIR / "other_spots.csv",
    BASE_DIR / "Datasets" / "other spots.csv",
    PROJECT_ROOT / "Datasets" / "other spots.csv",
    BACKEND_DIR / "Datasets" / "other spots.csv",
    Path.cwd() / "Datasets" / "other spots.csv",
    Path.cwd().parent / "Datasets" / "other spots.csv"
]
spots_csv = next((p for p in spots_csv_candidates if p.exists()), DATA_DIR / "other spots.csv")

amenities_csv_candidates = [
    DATA_DIR / "nearby_amenities.csv",
    BASE_DIR / "Datasets" / "nearby_amenities.csv",
    PROJECT_ROOT / "Datasets" / "nearby_amenities.csv",
    Path.cwd() / "Datasets" / "nearby_amenities.csv"
]
amenities_csv = next((p for p in amenities_csv_candidates if p.exists()), DATA_DIR / "nearby_amenities.csv")

acc_csv_candidates = [
    DATA_DIR / "accommodations.csv",
    BASE_DIR / "Datasets" / "accommodations.csv",
    PROJECT_ROOT / "Datasets" / "accommodations.csv",
    Path.cwd() / "Datasets" / "accommodations.csv"
]
acc_csv = next((p for p in acc_csv_candidates if p.exists()), DATA_DIR / "accommodations.csv")

visitors_csv_candidates = [
    DATA_DIR / "spot_visitors.csv",
    BASE_DIR / "Datasets" / "spot_visitors.csv",
    PROJECT_ROOT / "Datasets" / "spot_visitors.csv",
    Path.cwd() / "Datasets" / "spot_visitors.csv"
]
visitors_csv = next((p for p in visitors_csv_candidates if p.exists()), DATA_DIR / "spot_visitors.csv")

spots_df = pd.read_csv(spots_csv) if spots_csv.exists() else pd.DataFrame()
...
print(f"[Recommendation Log] Script path: {Path(__file__).resolve()}")
print(f"[Recommendation Log] BACKEND_DIR: {BACKEND_DIR}")
print(f"[Recommendation Log] BASE_DIR: {BASE_DIR}")
print(f"[Recommendation Log] PROJECT_ROOT: {PROJECT_ROOT}")
print(f"[Recommendation Log] DATASET PATH: {spots_csv}")
print(f"[Recommendation Log] DATASET EXISTS: {spots_csv.exists()}")
print(f"[Recommendation Log] TOTAL SPOTS LOADED: {len(spots_df)}")
if not spots_df.empty and "district" in spots_df.columns:
    print(f"[Recommendation Log] COLUMNS: {spots_df.columns.tolist()}")
    print(f"[Recommendation Log] UNIQUE DISTRICTS: {len(spots_df['district'].unique())}")
    hyd_cnt = len(spots_df[spots_df['district'].astype(str).str.strip().str.casefold() == 'hyderabad'])
    bhd_cnt = len(spots_df[spots_df['district'].astype(str).str.strip().str.casefold() == 'bhadradri kothagudem'])
    print(f"[Recommendation Log] HYDERABAD SPOTS: {hyd_cnt}")
    print(f"[Recommendation Log] BHADRADRI SPOTS: {bhd_cnt}")
...
def get_all_districts() -> List[str]:
    ...
    if spots_df.empty or "district" not in spots_df.columns:
        raise RuntimeError(
            "Tourist spots dataset could not be loaded or is empty. "
            "Verify Datasets/other spots.csv exists in deployment."
        )
...
def get_spots(district: Optional[str] = None, category: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    if district and normalize_text(district) != "all":
        target_dist = normalize_text(district)
        df = df[df["district"].astype(str).apply(normalize_text) == target_dist]

    if category and normalize_text(category) != "all":
        target_cat = normalize_text(category)
        df = df[df["category"].astype(str).apply(normalize_text) == target_cat]
    ...
    for _, row in df.iterrows():
        ...
        try:
            reviews_cnt = int(reviews_val)
        except Exception:
            reviews_cnt = 1500

        spots.append({
            "name": str(row.get("name", "Tourist Spot")),
            "district": str(row.get("district", district or "Hyderabad")).title(),
            "category": str(row.get("category", "heritage")).lower(),
            "rating": float(row.get("rating", 4.5)),
            "popularity": float(row.get("popularity", 80.0)),
            "entry_fee": float(row.get("entry_fee", 20.0)),
            "lat": lat,
            "lon": lon,
            "reviews": reviews_cnt
        })
    ...
    spots = sorted(spots, key=lambda s: (s["rating"], s["popularity"]), reverse=True)
    return spots
